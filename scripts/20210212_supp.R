...
test <- newdose %>%
    dplyr::mutate(drug = condition,
                  condition = paste0(drug, "_", dose_uM),
                  control = ifelse(condition == "abamectin_0", "None", "abamectin_0")) %>%
    dplyr::rename(dose_nM = dose_uM) %>%
    dplyr::select(date:plate, drug, condition:phenotype)
newdose <- test
...
for(t in unique(newdose$trait)) {
    ...
    dosepruned3 <- dosepruned2 %>%
        ...
        dplyr::ungroup() %>%
        dplyr::select(drug, strain, mean_ctrl) %>%
        dplyr::full_join(dosepruned2, by = c("drug", "strain")) %>%
        dplyr::distinct() %>%
...
dosepruned_sub %>%
    ...
    dplyr::group_by(drug, trait, dose) %>%
    dplyr::mutate(phen = max(newpheno, na.rm = T)) %>%
    dplyr::group_by(drug, trait, dose, strain) %>%
    ...
    tidyr::drop_na(drug) %>%
    dplyr::arrange(dose) %>%
    dplyr::distinct(drug, trait, dose, strain, .keep_all = T) %>%
    ggplot2::ggplot(.) +
...
#          Fig S6 - CSSV            #
#####################################
# 20210121 - altered to add DMSO phenotypes
...
regressed <- easysorter::regress(cssVpruned %>%
    ...
    dplyr::mutate(strain_fill = dplyr::case_when(strain %in% c("N2") ~ "N2",
                                                 strain %in% c("CB4856") ~ "CB",
                                                 TRUE ~ "NIL"),
                  condition = "regressed-abamectin") %>%
    dplyr::filter(trait == trt) %>%
    dplyr::mutate(rel_pheno = ((phenotype - min(phenotype, na.rm = T)) / (max(phenotype, na.rm = T) - min(phenotype, na.rm = T)))) %>%
    dplyr::mutate(phenotype = rel_pheno)


stats <- get_stats(regressed) %>%
    dplyr::mutate(condition = "regressed-abamectin") %>%
    dplyr::bind_rows(get_stats(pruned)) %>%
    dplyr::bind_rows(get_stats(dmso)) %>%
    dplyr::filter(comparison %in% c("N2-ECA573", "ECA554-CB4856"),
                  trait == trt)%>%
...
all_data <- regressed %>%
    ...
    dplyr::bind_rows(dmso)

plots <- plot_nil(all_data, nil_genotypes, stats, strainset, "V", ylab = "Mean length", supp = T)

genoplot <- cowplot::plot_grid(plots[[1]], plots[[2]], nrow = 1, rel_widths = c(3, 1), align = "h", axis = "b", labels = c("A", ""))
phenoplot <- cowplot::plot_grid(plots[[3]], nrow = 1, align = "h", axis = "b", labels = c("B", "", ""))
cowplot::plot_grid(genoplot, phenoplot, nrow = 2, align = "v", axis = "l")

# save
ggsave("figures/FigS6_cssV.png", height = 7.5, width = 7.5)
...
stats <- get_stats(regressed) %>%
    ...
    dplyr::bind_rows(get_stats(pruned)) %>%
    dplyr::bind_rows(get_stats(dmso)) %>%
    dplyr::filter(grepl("N2", comparison),
...
plots <- plot_nil(all_data, nil_genotypes, stats, strainset, "V", ylab = "Mean optical density", supp = T)
...
# save
ggsave("figures/FigS7_chrVnil.png", height = 7.5, width = 7.5)



#####################################
#  Fig S8 -fine map with NIL        #
#####################################
...
ggsave(plot, filename = "figures/FigS8_finemap_NIL.png", height = 5, width = 7.5)
...
ggsave("figures/FigS9_mediation.png", height = 4, width = 7.5)
...
# save
ggsave("figures/FigS10_VCnil.png", height = 7.5, width = 7.5)
...
# save
ggsave("figures/FigS11_VCnil.png", height = 7.5, width = 7.5)
